...
readonly DEFAULT_INTERNAL_PORT=38060
...
readonly HAPROXY_SERVER_NAME="fetch-mcp"
...
validate_fetch_env() {
    ...
    if [[ -n "${DEFAULT_LIMIT:-}" ]]; then
        if ! is_non_negative_int "$DEFAULT_LIMIT"; then
            echo "Warning: Invalid DEFAULT_LIMIT: '$DEFAULT_LIMIT'. Using default: $DEFAULT_LIMIT_VALUE" >&2
    ...
    if [[ -n "${FETCH_TIMEOUT:-}" ]]; then
        if ! is_positive_int "$FETCH_TIMEOUT"; then
            echo "Warning: Invalid FETCH_TIMEOUT: '$FETCH_TIMEOUT'. Using default: $DEFAULT_TIMEOUT" >&2
            export FETCH_TIMEOUT="$DEFAULT_TIMEOUT"
        elif [ "$FETCH_TIMEOUT" -lt 1000 ] || [ "$FETCH_TIMEOUT" -gt 300000 ]; then
            echo "Warning: FETCH_TIMEOUT out of reasonable range (1000-300000ms). Using default: $DEFAULT_TIMEOUT" >&2
            export FETCH_TIMEOUT="$DEFAULT_TIMEOUT"
        fi
    fi
    ...
    if [[ -n "${MAX_REDIRECTS:-}" ]]; then
        if ! is_non_negative_int "$MAX_REDIRECTS"; then
            echo "Warning: Invalid MAX_REDIRECTS: '$MAX_REDIRECTS'. Using default: $DEFAULT_MAX_REDIRECTS" >&2
            export MAX_REDIRECTS="$DEFAULT_MAX_REDIRECTS"
        elif [ "$MAX_REDIRECTS" -gt 20 ]; then
            echo "Warning: MAX_REDIRECTS too high ($MAX_REDIRECTS). Using default: $DEFAULT_MAX_REDIRECTS" >&2
            export MAX_REDIRECTS="$DEFAULT_MAX_REDIRECTS"
        fi
    fi
    ...
    if [[ -n "${FOLLOW_REDIRECTS:-}" ]]; then
        ...
        if [[ "$follow_redirects_lower" != "true" && "$follow_redirects_lower" != "false" ]]; then
            echo "Warning: Invalid FOLLOW_REDIRECTS: '$FOLLOW_REDIRECTS'. Using true." >&2
    ...
    if [[ -n "${VERIFY_SSL:-}" ]]; then
        ...
        if [[ "$verify_ssl_lower" != "true" && "$verify_ssl_lower" != "false" ]]; then
            echo "Warning: Invalid VERIFY_SSL: '$VERIFY_SSL'. Using true." >&2
...
start_mcp_server() {
    ...
    # MCP_SERVER_CMD is built by build_mcp_server_cmd(). Split preserving quoted segments.
    local -a backend_argv
    # shellcheck disable=SC2086
    eval "backend_argv=(${MCP_SERVER_CMD})"
    ...
    echo "Launching Fetch MCP Server via ${PROTOCOL_DISPLAY}"
...
main() {
    ...
    if ! validate_fetch_env; then
        echo "Fetch MCP Server cannot start due to configuration errors." >&2
        exit 1
    fi
