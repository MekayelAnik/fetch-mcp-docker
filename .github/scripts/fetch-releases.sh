...
NPM_PACKAGE="${NPM_PACKAGE:-fetch-mcp}"
...
if [[ -n "$MANUAL_VERSIONS_RAW" && "$REQUESTED_ACTION" == "build-versions" ]]; then
    ...
    if [[ -z "$VERSION_RANGE" ]]; then
        echo "::error::build-range action requires a version_range input (e.g. 1.1.0-1.1.2)"
        echo "versions_json=[]" >> "$GITHUB_OUTPUT"
        echo "latest_version=" >> "$GITHUB_OUTPUT"
        echo "should_build=false" >> "$GITHUB_OUTPUT"
        exit 1
    ...
    if [[ "$RANGE_START" == "$VERSION_RANGE" || "$RANGE_END" == "$VERSION_RANGE" ]]; then
        echo "Invalid version range format: '$VERSION_RANGE'. Expected format: X.Y.Z-A.B.C (e.g. 1.1.0-1.1.2)" >&2
        echo "versions_json=[]" >> "$GITHUB_OUTPUT"
        echo "latest_version=" >> "$GITHUB_OUTPUT"
        echo "should_build=false" >> "$GITHUB_OUTPUT"
        exit 1
    ...
    if [[ "$RANGE_LOWER" != "$RANGE_START" ]]; then
        echo "::error::Version range is reversed: $RANGE_START is greater than $RANGE_END. Use format: LOWER-HIGHER (e.g. 1.1.0-1.1.2)"
        echo "versions_json=[]" >> "$GITHUB_OUTPUT"
        echo "latest_version=" >> "$GITHUB_OUTPUT"
        echo "should_build=false" >> "$GITHUB_OUTPUT"
        exit 1
